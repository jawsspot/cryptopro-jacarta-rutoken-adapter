...
export class RutokenDriver {
    ...
    #checkVersion(lastVersion) {
        let _this = this
        if (this.#plugin.version.toString() < lastVersion)
            _this.pluginVersionStatus = `Версия вашего плагина устарела. Новая версия: ${lastVersion}`
        else
            _this.pluginVersionStatus = 'У Вас последняя версия плагина'
    }
    ...
    async #getAllCertificates() {
        try{
            ...
            throw this.#handleError(4, this.#errors[reason.message] || reason.message, 'getAllCertificates')
        }
    }
    ...
    async #bindToken(deviceId) {
        try {
            ...
            await this.#plugin.login(deviceId, window.SADO_TOKEN_PIN ?? prompt("Введите pin"))
